...
import pages.Map_window  as Map


theme = Theme()
styles = theme.tk_styles()

fileList = [] #DO NOT TOUCH (this is the list that stores file paths)

# this code is executed when the generate button is pressed
def generate(fileList):
    print(fileList) #prints the photo paths (for debugging) left it here just in case
    Map.Map_window().create(fileList)
...
class UploadingPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # button frame for holding 2 buttons
        ...
        back_button.pack(side=tk.LEFT,padx= 10,pady=10) #positions the button left

        # generate button
        next_button = tk.Button(button_frame, text= "generate",
                                command= lambda: generate(fileList)
                                )
        next_button.config(**styles["primary_button"])
        next_button.pack(side=tk.RIGHT,padx=10,pady=10) # positions the button right
    ...
    def handle_delete(self, file_path: str, item_frame: tk.Frame):
        ...
        if file_path in fileList:
            fileList.remove(file_path)
            print("Removed:", file_path)
            print("Current fileList:", fileList)
